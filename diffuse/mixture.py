...
def pdf_mixtr(state:MixState, x):
    ...
    pdfs = jax.scipy.stats.norm.pdf(x, mu, sigma)
    return  weights @ pdfs
...
def sampler_mixtr(key, state:MixState, N):
    ...
    noise = jax.random.normal(key2, shape=(N,))
    return mu[idx] + jnp.einsum('nd, n->nd', sigma[idx], noise)
...
def display_histogram(samples, ax):
    ...
    ax.bar(jnp.linspace(-xmax, xmax, nbins), h0, width=2*xmax/(nbins-1), align='center', color='red')
...
if __name__ == "__main__":
    ...
    # noise samples
    schedule = jnp.linspace(0.001, .999, 100)
