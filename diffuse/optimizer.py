...
def particle_step(sde_state, rng_key, drift_y, cond_sde, dt, y, ys_next):
    ...
    lgpdf = partial(
        logpdf_change_y, y=y, y_next=ys_next, drift_y=drift_y, cond_sde=cond_sde, dt=dt
    )
...
def impl_step(
    ...
    position, weights = jax.vmap(update_joint)(
    ...
    def update_expected_posterior(cntrst_sde_state, ys, ys_next, key):
        ...
        positions = particle_step(
            cntrst_sde_state, key, drift_y, cond_sde, dt, ys, ys_next
        )
        return positions
    ...
    # pdb.set_trace()
    position, weights = jax.vmap(update_expected_posterior)(
        cntrst_sde_state, ys[:-1], ys[1:], keys_time_c
    )
    ...
    grad_xi, ys = grad_xi_score(thetas[-1], cntrst_thetas[-1], design, cond_sde)
