from typing import Protocol, NamedTuple
from jaxtyping import Array, PRNGKeyArray

class MeasurementState(NamedTuple):
    y: Array
    mask_history: Array

class ForwardModel(Protocol):
    std: float

    def projection_design(self, design: Array) -> Array: ...

    def measure(self, design: Array, theta: Array) -> Array: ...

    def measure_from_mask(self, rng_key: PRNGKeyArray, mask: Array, x: Array) -> Array: ...

    def restore(self, design: Array, y: Array, new_measurement: Array) -> Array: ...

    def restore_from_mask(self, mask: Array, new_measurement: Array) -> Array: ...

    def make(self, design: Array) -> Array: ...

    def init_design(self, rng_key: PRNGKeyArray) -> Array: ...

    def init_measurement(self, rng_key: PRNGKeyArray) -> MeasurementState: ...

    def update_measurement(
        self, measurement_state: MeasurementState, new_measurement: Array, design: Array
    ) -> MeasurementState: ...

    def logprob_y(self, theta: Array, y: Array, design: Array) -> Array: ...

    def logprob_y_t(self, theta: Array, y_t: Array, mask: Array, alpha_t: float) -> Array: ...

    def grad_logprob_y(self, theta: Array, y: Array, mask: Array) -> Array: ...
