...
def Train(csv, cfg, preload=True):
    ...
    for _, sub in csv.iterrows():
        ...
        if sub.mask_path != None:  # if we have masks
            ...
            subject_dict["mask"] = tio.LabelMap(
                tensor=tio.ScalarImage(sub.img_path, reader=sitk_reader).data > 0
            )
    ...
    if preload:
        ...
        ds = tio.SubjectsDataset(
            subjects, transform=tio.Compose([get_transform(cfg), get_augment(cfg)])
        )
...
def Eval(csv, cfg):
    ...
    for _, sub in csv.iterrows():
        ...
        subject_dict = {
            "vol": tio.ScalarImage(sub.img_path, reader=sitk_reader),
            "vol_orig": tio.ScalarImage(
                sub.img_path, reader=sitk_reader
        ...
        if sub.seg_path != None:  # if we have segmentations
            ...
            subject_dict["seg_orig"] = tio.LabelMap(
                sub.seg_path, reader=sitk_reader
            )  # we need the image in original size for evaluation
            subject_dict["seg_available"] = True
        if sub.mask_path != None:  # if we have masks
            subject_dict["mask"] = tio.LabelMap(sub.mask_path, reader=sitk_reader)
            subject_dict["mask_orig"] = tio.LabelMap(
                sub.mask_path, reader=sitk_reader
            )  # we need the image in original size for evaluation
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.onlyBrain:
            ...
            for i in range(subject["vol"].data.shape[-1]):
                if (
                    subject["mask"].data[0, :, :, i].any() and start_ind is None
                ):  # only do this once
                    start_ind = i
                if (
                    not subject["mask"].data[0, :, :, i].any() and start_ind is not None
                ):  # only do this when start_ind is set
                    stop_ind = i
            low = start_ind
            high = stop_ind
        else:
        ...
        if self.slice is not None:
            ...
            if self.cfg.get(
                "unique_slice", False
            ):  # if all slices in one batch need to be at the same location
                if (
                    self.counter % self.cfg.batch_size == 0 or self.ind is None
                ):  # only change the index when changing to new batch
                    self.ind = torch.randint(low, high, size=[1])
        ...
        preprocess = tio.Compose(
            [
                ...
                tio.RescaleIntensity(
                    (0, 1),
                    percentiles=(cfg.get("perc_low", 1), cfg.get("perc_high", 99)),
                    masking_method="mask",
                ),
                tio.Resample(
...
def sitk_reader(path):
    ...
    if not "mask" in str(path) and not "seg" in str(
        path
    ):  # only for volumes / scalar images
        image_nii = sitk.CurvatureFlow(
            image1=image_nii, timeStep=0.125, numberOfIterations=3
        )
