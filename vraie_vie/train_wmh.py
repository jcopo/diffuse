...
def step(key, params, opt_state, ema_state, data, optimizer, ema_kernel, sde, cfg):
    val_loss, g = jax.value_and_grad(loss)(params, key, data, sde, cfg['n_t'], cfg['tf'])
...
if __name__ == "__main__":
    ...
    init_params = nn_unet.init(
        key, jnp.ones((config["batch_size"], *train_loader.dataset[0].shape)), jnp.ones((config["batch_size"],))
    )
    ...
    batch_update = partial(step, optimizer=optimizer, ema_kernel=ema_kernel, sde=sde, cfg=config)
    ...
    for epoch in range(config['n_epochs']):
        ...
        for batch in iterator:
            ...
            params, opt_state, ema_state, val_loss, ema_params = batch_update(subkey, params, opt_state, ema_state, batch)

            iterator.set_postfix({"loss": val_loss})
            list_loss.append(val_loss)

        print(f"Epoch {epoch}, loss {val_loss}, mean_loss {sum(list_loss) / len(list_loss)}")

        if epoch % 5 == 0:
            np.savez(os.path.join(config["save_path"], f"ann_{epoch}.npz"), params=params, ema_params=ema_params)

    np.savez(os.path.join(config["save_path"], f"ann_end.npz"), params=params, ema_params=ema_params)