...
def create_model_and_sde():
    ...
    # SDE setup
    beta = LinearSchedule(
        b_min=CONFIG["sde"]["b_min"],
        b_max=CONFIG["sde"]["b_max"],
        t0=CONFIG["sde"]["t0"],
        T=CONFIG["sde"]["T"]
    )
...
def create_optimizer():
    ...
    warmup_schedule = optax.linear_schedule(
        init_value=0.0,
        end_value=CONFIG["training"]["lr_init"],
...
def train_step(key, params, opt_state, ema_state, batch, model, sde, optimizer, ema_kernel):
    ...
    def loss_fn(params):
        return score_match_loss(
            params, key, batch, sde, model,
            lmbda=lambda t: weight_function(t, sde)
        )
    ...
    for epoch in range(CONFIG["training"]["n_epochs"]):
        ...
        for batch_idx in pbar:
            ...
            params, opt_state, ema_state, loss_val, ema_params = train_step(
                key_step, params, opt_state, ema_state, batch,
                model, sde, optimizer, ema_kernel
            )
