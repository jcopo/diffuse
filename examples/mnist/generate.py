...
CONFIG = {
    "schedules": {
        "LinearSchedule": {
            "params": {"b_min": 0.02, "b_max": 5.0, "t0": 0.0, "T": 2.0},
    ...
    "integrators": {
        ...
        "heun": (HeunIntegrator, {
            "stochastic_churn_rate": 0.,
            "churn_min": 0.5,
            "churn_max": 2.0,
            "noise_inflation_factor": 1.0001
        }),
        "dpmpp2s": (DPMpp2sIntegrator, {
            "stochastic_churn_rate": 0.,
            "churn_min": 0.5,
            "churn_max": 2.0,
            "noise_inflation_factor": 1.0001
...
def run_unconditional_experiment(
    key: jax.random.PRNGKey,
    integrator_type: str = "dpmpp2s"
):
    ...
    # Create timer
    timer = VpTimer(
        n_steps=CONFIG["space"]["n_steps"],
        eps=CONFIG["space"]["t_init"],
        tf=CONFIG["space"]["t_final"]
    )
    ...
    state, hist = denoiser.generate(
        key_samples,
        CONFIG["space"]["n_steps"],
        CONFIG["space"]["n_samples"]
    )
...
def run_conditional_experiment(
    key: jax.random.PRNGKey,
    denoiser_type: str = "dps",
    integrator_type: str = "euler_maruyama",
    mask_size: int = 7
):
...
if __name__ == "__main__":
    ...
    state, hist = run_unconditional_experiment(
        key,
        integrator_type="dpmpp2s"
    )
    ...
    state, hist = run_conditional_experiment(
        key,
        denoiser_type="dps",
        integrator_type="euler_maruyama",
        mask_size=7
    )
