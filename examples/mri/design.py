...
def initialize_experiment(key: PRNGKeyArray, config: dict):
    ...
    if config.get("training", {}).get("loss") in [
        "noise_matching",
        "mae_noise_matching",
    ]:
    ...
    if config["mask"]["mask_type"] == "spiral":
        ...
        mask = maskVertical(
            num_lines=10,  # config['mask']['num_lines'],
            img_shape=shape,
            task=config["task"],
            data_model=config["dataset"],
        )
...
def main(
    ...
    print(
        ...
        "\n \
            mask_type: ",
        config["mask"]["mask_type"],
        "\n \
            num_lines: ",
        config["mask"]["num_lines"],
        "\n \
            n_t: ",
        config["inference"]["n_t"],
        "\n \
            n_samples: ",
        config["inference"]["n_samples"],
        "\n \
            n_samples_cntrst: ",
        config["inference"]["n_samples_cntrst"],
        "\n \
            n_loop_opt: ",
        config["inference"]["n_loop_opt"],
        "\n \
            ",
        file=sys.stdout,
        flush=True,
    ...
    sde, mask, ground_truth, dt, n_t, nn_score, experiment = initialize_experiment(
        key, config
    )
    lpips = LPIPS()
    ...
    # ExperimentOptimizer
    optimizer = optax.chain(optax.adam(learning_rate=schedule), optax.scale(-1))
    ...
    def scan_step(carry, n_meas):
        ...
        if logger and len(devices) == 1:
            logger.log(ground_truth, optimal_state, measurement_state, history, n_meas)
    ...
    (exp_state, final_measurement_state, key), (optimal_states, measurement_states) = (
        jax.lax.scan(scan_step, init_carry, jnp.arange(num_measurements))
    )
        ...
        def log_all_steps(ground_truth, optimal_states, measurement_states):
            ...
            try:
                ...
                for step in range(num_measurements):
                    ...
                    step_measurement = jax.tree.map(
                        lambda x: x[step], measurement_states
                    )
